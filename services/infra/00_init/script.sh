...
if [ "$CLUSTER_TYPE" == "openshift" ]; then
    if ! oc get sa infra-sa >/dev/null 2>&1; then
        oc create sa infra-sa
        print_log "Service account infra-sa is created"
    fi
    oc adm policy add-scc-to-user anyuid -z infra-sa -n $NS || true
